...
//! Async display loop with time + IP + animation
...
fn get_eth02_ip() -> String {
    use std::process::Command;
    let output = Command::new("ip").args(["addr", "show", "eth0.2"]).output().ok();
...
/// Async display loop - updates OLED with time + IP + animation
